...
def install_msvc_redist(dl_dir, url=None):
    ...
    LOGGER.info("Downloading MSVC Redist from {}".format(url))
    ...
    build_utils.run_subprocess(command, LOGGER)
    os.remove(dl_file)
...
def install_loot_api(version, revision, dl_dir, destination_path):
    ...
    LOGGER.info("Downloading LOOT API Python wrapper from {}".format(url))
    LOGGER.debug("Downloading MSVC Redist to {}".format(archive_path))
    build_utils.download_file(url, archive_path)
    LOGGER.info("Extracting LOOT API Python wrapper to " + destination_path)
    command = [
        seven_zip_path,
        "e",
        archive_path,
