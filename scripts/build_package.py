...
def setup_parser(parser):
    ...
    parser.add_argument(
        ...
        "in [default: {}].".format(os.path.relpath(DIST_PATH, os.getcwd())),
    )
    parser.add_argument(
        "--no-standalone",
        action="store_false",
...
def get_version_info(version, padding=4):
    ...
    LOGGER.info("Using file version: {}".format(file_version))
    return file_version
...
def pack_7z(file_list, archive, list_path):
    ...
    try:
        build_utils.run_subprocess(cmd_7z, LOGGER, cwd=ROOT_PATH)
    finally:
        rm(list_path)
...
def get_nsis_root(cmd_arg):
    ...
    if not os.path.isdir(local_nsis_path):
        ...
        LOGGER.info("Downloading NSIS {} from {}".format(NSIS_VERSION, nsis_url))
        ...
        LOGGER.info("Downloading inetc plugin from {}".format(inetc_url))
...
def build_executable(version, file_version):
    ...
    try:
        ...
        build_utils.run_subprocess([upx, "-9", exe], LOGGER)
    except:
...
def main(args):
    build_utils.setup_log(LOGGER, verbosity=args.verbosity, logfile=args.logfile)
